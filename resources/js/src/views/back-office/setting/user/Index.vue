<template>
    <div>
                    ...
                    <div class="col-lg-12 pt-sm pb-5" ref="modalHapusContainer">
                        ...
                        <card shadow>
                            ...
                            <div class="table-responsive">
                                <a-table :pagination="pagination" :columns="columns" :data-source="filteredUser"
        ...
        <modal :show.sync="modals.tambah">
            <h6 slot="header" class="modal-title" id="modal-title-defaultt">Tambah Data User</h6>
            <TambahUser :users="users" @data-added="handleDataSubmit"></TambahUser>
            <template slot="footer">
                <button type="submit" class="btn btn-primary" form="proses-tambah-user"><i class="fa fa-send"></i> Simpan</button>
        ...
        <modal :show.sync="modals.edit">
            <h6 slot="header" class="modal-title" id="modal-title-defaulte">Edit Data User</h6>
            <EditUser :dataUpdate="selectedItem" @data-updated="handleDataSubmit"></EditUser>
            <template slot="footer">
                <button type="submit" class="btn btn-primary" form="proses-edit-user"><i class="fa fa-send"></i> Update</button>
...
import TambahUser from "./Tambah.vue";
...
export default {
    ...
    methods: {
        async fetchData() {
            try {
                const response = await axios.get('/api/users'); // Ambil data dari API
        ...
        async deleteData(id) {
            try {
                ...
                await axios.delete(`/api/users/${id}`); // Hapus data dari API
        ...
        openEditModal(record) {
            ...
            const target = this.users.find(item => item.id === record.id);
